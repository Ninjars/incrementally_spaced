...
public class ExplosionMat : MonoBehaviour {
	...
	void Start () {
		...
		renderComponent.material = new Material(ExplosionMaterial);
	...
	void Update () {
		if (doUpdate) {
			Material rsm = renderComponent.sharedMaterial;
	...
	void SetShaderKeywords () {
		...
		renderComponent.sharedMaterial.shaderKeywords = newKeywords.ToArray();
	}
}
